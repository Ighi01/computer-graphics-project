...
//	glm::mat4 M;
	...
	M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.0f,0,0,   0,0,1.0f/(0.1f-500.0f),0, 0,0,0.1f/(0.1f-500.0f),1);
	A->SetMatrix(0,  M);

	// Isometric
	M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.0f,0,0,   0,0,1.0f/(-500.0f-500.0f),0, 0,0,-500.0f/(-500.0f-500.0f),1)
	* glm::mat4(1.0f,  0, 0, 0, 0,0.8165f, 0.57728f, 0, 0, -0.57728f, 0.8165f, 0,0 ,0, 0, 1.0f)
	* glm::mat4(0.707f,  0, -0.707f, 0, 0, 1.0f,0, 0, 0.707f, 0, 0.707f, 0,0 ,0, 0, 1.0f);
	A->SetMatrix(1,  M);

	// Dimetric, with an angle of 20 degree
	M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.0f,0,0,   0,0,1.0f/(-500.0f-500.0f),0, 0,0,-500.0f/(-500.0f-500.0f),1)
	* glm::mat4(1.0f,  0, 0, 0, 0,0.93969f, 0.342f, 0, 0, -0.342f, 0.93969f, 0,0 ,0, 0, 1.0f)
	* glm::mat4(0.707f,  0, -0.707f, 0, 0, 1.0f,0, 0, 0.707f, 0, 0.707f, 0,0 ,0, 0, 1.0f);
	A->SetMatrix(2,  M);
	
	// Trimetric, with an angle of alpha of 30 degree, and beta of 60 degrees
	M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.0f,0,0,   0,0,1.0f/(-500.0f-500.0f),0, 0,0,-500.0f/(-500.0f-500.0f),1)
	* glm::mat4(1.0f,  0, 0, 0, 0,0.5f, 0.866f, 0, 0, -0.866f, 0.5f, 0,0 ,0, 0, 1.0f)
	* glm::mat4(0.866f,  0, -0.5f, 0, 0, 1.0f,0, 0, 0.5f, 0, 0.866f, 0,0 ,0, 0, 1.0f);
	A->SetMatrix(3,  M);
	
	// Create a Cabinet projection, with the z axis at an angle of 45 degrees
	M = glm::mat4(1.0f / 20.0f,0,0,0,  0,-4.0f / 60.0f,0,0,   0,0,1.0f/(-500.0f-500.0f),0, 0,0,-500.0f/(-500.0f-500.0f),1) 
	* glm::shearZ3D(glm::mat4(1.0f), -0.3535f, -0.3535f);
	A->SetMatrix(4,  M);
	
	// Create a perspective projection, with a Fov-y of  90 degrees
	M = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, -1.0f, 1.0f))
	* glm::frustum(-0.1333f, 0.1333f, -0.1f, 0.1f, 0.1f, 500.0f);
	A->SetMatrix(5,  M);
	
	// Create a perspective projection, with a Fov-y of  30 degrees (zoom)
	M = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, -1.0f, 1.0f))
	* glm::frustum(-0.0357f, 0.0357f, -0.02679f, 0.02679f, 0.1f, 500.0f);
	A->SetMatrix(6,  M);
	
	// Create a perspective projection, with a Fov-y of 120 degrees (wide)
	//M = glm::perspective(120.0f, 1.3333f, 0.1f , 500.0f);
	M = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, -1.0f, 1.0f))
	* glm::frustum(-0.23f, 0.23f, -0.173f, 0.173f, 0.1f, 500.0f);
	A->SetMatrix(7,  M);
